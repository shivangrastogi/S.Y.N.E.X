...
class SettingsPage(QWidget):
    ...
    tts_voice_save_clicked = pyqtSignal(str)   # emitted when Save is pressed
    ...
    download_model_clicked = pyqtSignal(str) # model_name
    delete_model_clicked = pyqtSignal(str)   # model_name
    
    # New Security/Wake/Sleep signals
    wake_word_security_changed = pyqtSignal(bool)
    sleep_mode_changed = pyqtSignal(bool)
    listening_sensitivity_changed = pyqtSignal(int)
    ...
    def setup_ui(self):
        ...
        # NOTE: we do NOT connect currentTextChanged here — voice only applies on Save
        self.tts_voice_combo.currentTextChanged.connect(self._on_voice_changed)
        
        self.voice_info_label = QLabel("")
        self.voice_info_label.setStyleSheet("color: #9ad4ff; font-size: 11px;")
        
        self.install_btn = QPushButton("Install Model")
        self.install_btn.setStyleSheet(self._mini_button_style())
        self.install_btn.setVisible(False)
        self.install_btn.clicked.connect(self._on_install_clicked)
        
        self.delete_btn = QPushButton("Delete")
        self.delete_btn.setStyleSheet(self._mini_button_style().replace("rgba(0, 212, 255, 0.1)", "rgba(255, 50, 50, 0.1)"))
        self.delete_btn.setVisible(False)
        self.delete_btn.clicked.connect(self._on_delete_clicked)

        self.voice_save_btn = QPushButton("💾  Save Voice")
        self.voice_save_btn.setStyleSheet("""
            QPushButton {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(0,212,255,0.25), stop:1 rgba(0,140,255,0.25));
            ...
            QPushButton:hover {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(0,212,255,0.45), stop:1 rgba(0,140,255,0.45));
                border-color: #00ffff;
            }
            QPushButton:pressed { background: rgba(0,212,255,0.6); }
        """)
        self.voice_save_btn.clicked.connect(self._on_voice_save)
        
        voice_container = QWidget()
        voice_row_layout = QVBoxLayout(voice_container)
        voice_row_layout.setContentsMargins(0, 0, 0, 0)
        voice_row_layout.addWidget(self.tts_voice_combo)
        voice_row_layout.addWidget(self.voice_info_label)
        
        btn_layout = QHBoxLayout()
        btn_layout.addWidget(self.install_btn)
        btn_layout.addWidget(self.delete_btn)
        btn_layout.addStretch()
        btn_layout.addWidget(self.voice_save_btn)
        voice_row_layout.addLayout(btn_layout)
        
        tts_card.add_option(SettingRow("Voice", voice_container))
        ...
        self.wake_security_toggle = SmoothToggleSwitch()
        self.wake_security_toggle.set_on(True, animate=False)
        self.wake_security_toggle.toggled.connect(self.wake_word_security_changed.emit)
        security_card.add_option(SettingRow("Wake Word Security", self.wake_security_toggle,
                                            "Require wake word 'JARVIS' for all commands"))
        
        self.sleep_mode_toggle = SmoothToggleSwitch()
        self.sleep_mode_toggle.set_on(False, animate=False)
        self.sleep_mode_toggle.toggled.connect(self.sleep_mode_changed.emit)
        security_card.add_option(SettingRow("Sleep Mode", self.sleep_mode_toggle,
                                            "Low power mode: JARVIS stays quiet until woken"))
        
        self.sensitivity_slider = QSlider(Qt.Orientation.Horizontal)
        self.sensitivity_slider.setRange(100, 1000)
        self.sensitivity_slider.setValue(300)
        self.sensitivity_slider.setStyleSheet(self._slider_style())
        self.sensitivity_slider.valueChanged.connect(self.listening_sensitivity_changed.emit)
        security_card.add_option(SettingRow("Mic Sensitivity", self.sensitivity_slider, "Low = hears more noise, High = needs loud speech"))
        ...
        # System Data Card
        system_card = SettingsCard("🖥️ System Data")
        
        self.storage_btn = QPushButton("📦  Storage Hub")
        self.storage_btn.setStyleSheet(self._button_style())
        self.storage_btn.clicked.connect(self._show_storage_hub)
        
        system_card.add_option(SettingRow("Disk Usage", self.storage_btn, "View what's taking up space in JARVIS"))
    ...
    def set_available_voices(self, voices: list):
        """
        Populate the voice combo box.
        voices: list of dicts like {"id": "...", "name": "...", "type": "system/hf", "installed": bool}
        """
        ...
        for v in voices:
            ...
            self._voice_data[v["id"]] = v
            
        self.tts_voice_combo.blockSignals(False)
        self._on_voice_changed(self.tts_voice_combo.currentText())

    def _on_voice_changed(self, text):
        """Updates info label and button visibility when user browses voices."""
        voice_id = self.tts_voice_combo.currentData()
        if not voice_id or not hasattr(self, '_voice_data') or voice_id not in self._voice_data:
            return

        voice = self._voice_data[voice_id]
        engine = voice.get("engine", "")

        if engine == "xtts":
            self.voice_info_label.setText("Consistent voice for English + Hindi  ✨ Recommended")
            self.install_btn.setVisible(False)
            self.delete_btn.setVisible(False)

        elif engine == "piper":
            lang_str = "Hindi" if voice.get("lang") == "hi" else "Custom"
            self.voice_info_label.setText(f"Piper offline model  •  {lang_str}  •  Works without internet")
            self.install_btn.setVisible(False)
            # Only allow deleting non-default models
            model_name = voice_id.split(":", 1)[-1]
            self.delete_btn.setVisible(True)
            self.delete_btn.setEnabled(True)
            self.delete_btn.setText(f"🗑 Delete {model_name}")

        elif engine == "system":
            self.voice_info_label.setText("Built-in Windows/System voice  •  Always available")
            self.install_btn.setVisible(False)
            self.delete_btn.setVisible(False)
    ...
    def _on_voice_save(self):
        """Apply the selected voice — only called when user clicks Save Voice."""
        voice_id = self.tts_voice_combo.currentData()
        if voice_id:
            self.tts_voice_changed.emit(voice_id)   # backward compat signal
            self.tts_voice_save_clicked.emit(voice_id)
            # Visual feedback
            self.voice_save_btn.setText("✅  Saved!")
            from PyQt5.QtCore import QTimer
            QTimer.singleShot(2000, lambda: self.voice_save_btn.setText("💾  Save Voice"))

    def _on_install_clicked(self):
        """Open dialog to install a custom Piper voice model."""
        from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLineEdit, QLabel, QDialogButtonBox
        dlg = QDialog(self)
        dlg.setWindowTitle("Install Custom Piper Voice")
        dlg.setMinimumWidth(480)
        dlg.setStyleSheet("""
            QDialog { background: #0d1b2e; color: #e5f6ff; }
            QLabel  { color: #9ad4ff; font-size: 12px; }
            QLineEdit {
                background: rgba(0,212,255,0.08); border: 1px solid rgba(0,212,255,0.4);
                border-radius:4px; color:#fff; padding:6px; font-size:12px;
            }
        """)
        lay = QVBoxLayout(dlg)
        lay.addWidget(QLabel("Piper .onnx model URL (from HuggingFace or direct link):"))
        url_edit = QLineEdit(); url_edit.setPlaceholderText("https://huggingface.co/.../model.onnx")
        lay.addWidget(url_edit)
        lay.addWidget(QLabel("Model name (folder to save as):"))
        name_edit = QLineEdit(); name_edit.setPlaceholderText("e.g.  Piper-Hindi-Female")
        lay.addWidget(name_edit)
        lay.addWidget(QLabel("<small>Tip: Find voices at huggingface.co/rhasspy/piper-voices</small>"))
        ...
        if dlg.exec_() == QDialog.DialogCode.Accepted:
            ...
            if url and name:
                ...
                self.download_model_clicked.emit(f"{name}|{url}")  # encode both fields

    def _on_delete_clicked(self):
        """Confirm then delete the selected Piper model."""
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.delete_model_clicked.emit(model_name)

    def update_install_progress(self, model_name: str, progress: int):
        """Update the Install button with progress (0-100)"""
        # For now, just update the text
        if progress < 100:
            self.install_btn.setText(f"Installing {progress}%")
        else:
    ...
    def _show_storage_hub(self):
        """Open the simplified storage distribution modal"""
        try:
            from aeris.ui_laptop.widgets.storage_modal import StorageModal
            modal = StorageModal(self)
            modal.exec_()
        except ImportError:
            print("StorageModal not found")
